...
```
   ...
   - |||coder|re| what is this code doing? - sends the text you copied last, prepended by "what is this code doing", and invokes the coder assistant to help frame the output format and preconceptions. 
...
hardware("time to process")  [fast = 20+ tokens/sec, medium = <10 tokens/sec. slow = <2tokens/sec]*
32gb ram and not using graphicss card(medium):
for 8gb gfx cards and 16gb ram(fast) 
for 12gb gfx cards and 16gb ram(fast) and 32or 32gb ram and 8gb vram(mediumn):
for 12gb gfx cards and 32gb ram(medium):
for 12gb gfx cards and 32gb ram(slow): 
for 24gb vram(fast):
for 24gb vram(medium):
for 24gb vram(slow):
